    ...
    public class FileUploadService
    {
        ...
        public async Task<string> UploadImageAsync(IBrowserFile file)
        {
            ...
            try
            {
                ...
                // ⭐ CORRECTION : Lecture complète du fichier pour éviter la corruption
                byte[] fileContent;
                using (var stream = file.OpenReadStream(_maxFileSizes["image"]))
                {
                    ...
                    {
                        throw new InvalidOperationException("Le fichier est vide ou corrompu");
                    }
                    
                    if (fileContent.Length != file.Size)
                    {
                        _logger.LogWarning("Taille attendue: {Expected}, taille lue: {Actual}", file.Size, fileContent.Length);
                    }
                }
                
                // ⭐ Validation étendue des signatures d'images
                if (!IsValidImageFile(fileContent, file.ContentType))
                {
                    throw new InvalidOperationException("Le fichier n'est pas une image valide ou est corrompu");
                }
                ...
                if (File.Exists(filePath))
                {
                    ...
                    if (fileInfo.Length != fileContent.Length)
                    {
                        File.Delete(filePath);
                        throw new InvalidOperationException("Échec de l'écriture du fichier - corruption détectée");
                    }
                }
        ...
        // ⭐ NOUVELLE MÉTHODE : Validation complète des images
        private bool IsValidImageFile(byte[] fileContent, string contentType)
        {
            if (fileContent == null || fileContent.Length < 4)
                return false;
            
            // Vérifier les magic bytes selon le type
            var isValidSignature = contentType?.ToLower() switch
            {
                "image/jpeg" => fileContent[0] == 0xFF && fileContent[1] == 0xD8 && fileContent[2] == 0xFF,
                "image/png" => fileContent.Length >= 8 && 
                ...
                "image/gif" => fileContent.Length >= 6 &&
                              fileContent[0] == 0x47 && fileContent[1] == 0x49 && fileContent[2] == 0x46 &&
                              (fileContent[3] == 0x38 && (fileContent[4] == 0x37 || fileContent[4] == 0x39)),
                "image/webp" => fileContent.Length >= 12 &&
                               fileContent[0] == 0x52 && fileContent[1] == 0x49 && fileContent[2] == 0x46 && fileContent[3] == 0x46 &&
                               fileContent[8] == 0x57 && fileContent[9] == 0x45 && fileContent[10] == 0x42 && fileContent[11] == 0x50,
                "image/bmp" => fileContent.Length >= 2 && fileContent[0] == 0x42 && fileContent[1] == 0x4D,
                _ => false
            };
